...
const punjabiChannels = [
  { name: "PTC Punjabi", category: "Entertainment", emoji: "🎭" },
  { name: "Zee Punjabi", category: "Entertainment", emoji: "⭐" },
  { name: "MH1", category: "Entertainment", emoji: "📺" },
  { name: "Pitaara TV", category: "Entertainment", emoji: "🌟" },
  { name: "Colors Punjabi", category: "Entertainment", emoji: "🎨" },
  { name: "DD Punjabi", category: "Entertainment", emoji: "🏛️" },
  { name: "Chardikala Time TV", category: "Religious", emoji: "🙏" },
  { name: "Sikh Channel", category: "Religious", emoji: "☬" },
  { name: "PTC News", category: "News", emoji: "📰" },
  { name: "News18 Punjab", category: "News", emoji: "📡" },
  { name: "Zee News Punjab", category: "News", emoji: "📻" },
  { name: "ABP Sanjha", category: "News", emoji: "🎙️" },
  { name: "9X Tashan", category: "Music", emoji: "��" },
  { name: "MTV Beats", category: "Music", emoji: "🎶" },
  { name: "B4U Music", category: "Music", emoji: "🎼" },
  { name: "Punjabi Hit Music", category: "Music", emoji: "🎸" },
  { name: "Channel Punjab", category: "Music", emoji: "🎤" },
  { name: "PTC Movies", category: "Movies", emoji: "��" },
  { name: "Zee Cinema Punjab", category: "Movies", emoji: "🎞️" },
  { name: "B4U Kadak", category: "Movies", emoji: "🎥" },
  { name: "Punjabi Cinema", category: "Movies", emoji: "🎦" },
  { name: "ETC Punjabi", category: "Movies", emoji: "📽️" },
  { name: "Star Sports 1 Hindi", category: "Sports", emoji: "⚽" },
  { name: "PTC Sports", category: "Sports", emoji: "🏆" },
  { name: "Willow Cricket", category: "Sports", emoji: "🏏" },
  { name: "Sony Sports", category: "Sports", emoji: "🏀" },
];

const categories = ["All", "Entertainment", "News", "Music", "Movies", "Religious", "Sports"];
...
export default function PunjabiChannelsPage() {
  ...
  const filtered = activeCategory === "All"
    ? punjabiChannels
    : punjabiChannels.filter(c => c.category === activeCategory);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const msg = `📺 Punjabi Channel List Request!%0AName: ${form.name}%0APhone: ${form.phone}%0AEmail: ${form.email}%0AMessage: ${form.message}`;
  ...
  return (
    <main style={{ background: "#08080f", minHeight: "100vh", color: "#fff", overflowX: "hidden" }}>
      ...
      <div style={{ position: "relative", paddingTop: "220px", paddingBottom: "80px", textAlign: "center", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, backgroundImage: "url('/images/punjabi-hero.jpg')", backgroundSize: "cover", backgroundPosition: "center", opacity: 0.15 }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg,#0f0f1a,#1a0500,#08080f)" }} />
        <div style={{ position: "absolute", inset: 0, background: "rgba(8,8,15,0.75)" }} />
        <div style={{ position: "absolute", top: "30%", left: "50%", transform: "translate(-50%,-50%)", width: "700px", height: "500px", background: "radial-gradient(circle,rgba(255,165,0,0.1) 0%,transparent 70%)", pointerEvents: "none" }} />
        <div style={{ position: "relative", zIndex: 10 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,165,0,0.1)", border: "1px solid rgba(255,165,0,0.3)", borderRadius: "50px", padding: "8px 18px", marginBottom: "20px" }}>
            <span style={{ width: "8px", height: "8px", background: "#4ade80", borderRadius: "50%", display: "inline-block", boxShadow: "0 0 8px #4ade80" }} />
            <span style={{ color: "#FFA500", fontSize: "12px", fontWeight: 700, letterSpacing: "3px", textTransform: "uppercase" }}>🌾 Punjabi Channels</span>
          </div>
          ...
          <p style={{ color: "#9ca3af", fontSize: "1.1rem", maxWidth: "560px", margin: "0 auto 28px", lineHeight: 1.7 }}>
            Best Punjabi IPTV Channels Provider in USA, UK & Canada — 198+ Punjabi channels in HD & 4K quality
          </p>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", color: "#6b7280", fontSize: "14px", marginTop: "24px" }}>
            ...
            <span>/</span>
            <span style={{ color: "#FFD700", fontWeight: 600 }}>Punjabi</span>
          </div>
        </div>
      </div>
      ...
      <section style={{ padding: "60px 24px", maxWidth: "1200px", margin: "0 auto" }}>
        <h2 style={{ fontSize: "clamp(1.6rem,3vw,2.4rem)", fontWeight: 900, color: "#fff", marginBottom: "20px", lineHeight: 1.2 }}>
          ...
          Over 198+ Punjabi Channels with Krishna IPTV
        </h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "32px" }} className="resp-2col">
          <div>
            <p style={{ color: "#9ca3af", fontSize: "15px", lineHeight: 1.8, marginBottom: "16px" }}>
              Are you looking for a never-ending source of Punjabi entertainment? With <strong style={{ color: "#FFD700" }}>Krishna IPTV</strong>, you can instantly access <strong style={{ color: "#fff" }}>198+ premium Punjabi channels</strong>. These channels offer a constant stream of HD-quality movies, music, news, comedy, and cultural presentations — delivering your favorite regional entertainment straight to your living room in <strong style={{ color: "#fff" }}>USA</strong>, UK, or anywhere in the world.
            </p>
            <p style={{ color: "#9ca3af", fontSize: "15px", lineHeight: 1.8 }}>
              Every member of the family will find something they enjoy — popular dramas, reality shows, and live <strong style={{ color: "#fff" }}>Gurbani and Punjabi</strong> movies. Regardless of your preference for <strong style={{ color: "#fff" }}>PTC Punjabi, Zee Punjabi, or 9X Tashan</strong>, our selection is made to keep you occupied all day and night.
            </p>
          </div>
          <div>
            <p style={{ color: "#9ca3af", fontSize: "15px", lineHeight: 1.8, marginBottom: "16px" }}>
              Simply plug, play, and stream — no agreements, no satellite dish. Discover the most dependable <strong style={{ color: "#fff" }}>Punjabi IPTV channels</strong> available on the market and experience the future of Desi entertainment. Do not pass up the greatest opportunity to maintain your ties to your family!
            </p>
            <p style={{ color: "#9ca3af", fontSize: "15px", lineHeight: 1.8 }}>
              Not just Punjabi — our lineup incorporates <strong style={{ color: "#FFD700" }}>8,000+ channels</strong> in <strong style={{ color: "#fff" }}>Hindi, Tamil, Telugu, Malayalam, Bengali, Gujarati & Marathi</strong>. All plans come with a money-back guarantee with <strong style={{ color: "#fff" }}>24/7 support</strong> and instant activation.
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "60px 24px", background: "#0a0a12", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "12px" }} className="resp-4col">
            {filtered.map((ch, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 16px", background: "#0f0f1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "12px", transition: "all 0.2s" }}
                ...
                <div style={{ width: "36px", height: "36px", borderRadius: "8px", background: "rgba(255,165,0,0.1)", border: "1px solid rgba(255,165,0,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", flexShrink: 0 }}>{ch.emoji}</div>
                <div>
          ...
          <div style={{ textAlign: "center", marginTop: "32px" }}>
            <p style={{ color: "#6b7280", fontSize: "14px", marginBottom: "16px" }}>+ Many more Punjabi channels available. Contact us for full list!</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "32px" }} className="resp-2col">
            ...
            <div style={{ background: "linear-gradient(135deg,#0f0f1a,#1a1a2e)", border: "2px dashed rgba(255,215,0,0.12)", borderRadius: "20px", minHeight: "400px", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ textAlign: "center", color: "rgba(255,215,0,0.2)", padding: "32px" }}>
                ...
                <div style={{ fontSize: "15px", fontWeight: 700 }}>Punjabi Family Image</div>
                <div style={{ fontSize: "12px", opacity: 0.6, marginTop: "8px" }}>public/images/punjabi-family.jpg</div>
              </div>
            </div>
          </div>
        </div>
